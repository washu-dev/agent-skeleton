...
Subcommands:
    ...
    python -m agent_skeleton.serve serve-a2a      # OPTION 2: run the tool-loop agent (demo tools)
    python -m agent_skeleton.serve serve-handler \\
        --file handler.py --class MyHandler       # OPTION 3: run a custom handler locally
"""
...
# ``validate_tool_registry`` and ``HandlerExecutor`` live in the path packages
# (tool_loop / custom); they are imported lazily inside the functions that need
# them so ``agent_skeleton.core`` stays a clean, self-contained base layer that
# never imports "upward" into a path folder at module load.
...
def create_app(agent_card: Any, model: str | None = None, spec: Any = None) -> Any:
    """Build the OPTION 2 (tool-loop) ASGI app for a card + optional spec."""
    require_a2a()
...
# --- OPTION 3: run a custom AgentHandler locally --------------------------
...
def build_parser() -> argparse.ArgumentParser:
    ...
    serve = sub.add_parser("serve-a2a", help="OPTION 2: run the tool-loop agent (demo tools).")
    ...
    handler = sub.add_parser("serve-handler", help="OPTION 3: run a custom AgentHandler locally.")
